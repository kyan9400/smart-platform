...
export default function HeroSection() {
  ...
  return (
    <section className="relative bg-saudi-950 text-white overflow-hidden min-h-[580px] flex items-center">
      <SaudiHeroBackdrop />

      <div className="absolute -top-20 end-0 w-72 h-72 rounded-full bg-emerald-400/15 blur-3xl" />
      <div className="absolute bottom-0 start-0 w-64 h-64 rounded-full bg-amber-300/15 blur-2xl" />

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="text-start">
            <div className="inline-flex items-center gap-2 bg-yellow-400/20 border border-yellow-400/40 rounded-full px-4 py-1 mb-5 text-yellow-200 text-sm font-medium">
            ...
            <p className="text-emerald-100 text-base sm:text-lg leading-relaxed mb-3 max-w-lg">
              {t.hero.subtitle}
            </p>
            <p className="text-emerald-100/90 text-base sm:text-lg leading-relaxed mb-8 max-w-lg">
              {t.hero.body}
            </p>
            <div className="flex flex-wrap gap-3">
              <a
                href="#equipment"
                className="inline-flex items-center gap-2 bg-yellow-400 text-saudi-900 px-6 py-3 rounded-lg font-bold text-sm hover:bg-yellow-300 transition-all hover:shadow-lg hover:shadow-yellow-400/30 active:scale-95"
              >
                {t.hero.browse}{" "}
          ...
          <div className="flex justify-center lg:justify-end rtl:lg:justify-start">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
              <p className="text-emerald-100 text-xs font-semibold uppercase tracking-widest mb-4 text-start">
                ...
                {t.hero.stats.map(({ label, value }, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-emerald-100/90 text-xs mt-0.5">{label}</p>
                    </div>
                  );
                })}
              </div>
              <div className="mt-4 bg-yellow-400/20 border border-yellow-400/30 rounded-xl p-3 flex items-center gap-3">
                <div className="w-8 h-8 bg-yellow-400 rounded-full flex items-center justify-center text-saudi-900 font-bold text-xs shrink-0">
                  ✓
                </div>
                <p className="text-yellow-100 text-xs text-start leading-relaxed">
                  {t.hero.trustNote}
                </p>
              </div>
            </div>
